...
public interface ScheduleConflictRepository extends JpaRepository<ScheduleConflict, Integer> {
    ...
    @Query("SELECT sc FROM ScheduleConflict sc WHERE sc.schedule.period.id = :periodId AND sc.isResolved = false ORDER BY sc.createdAt DESC")
    List<ScheduleConflict> findUnresolvedByPeriodId(@Param("periodId") Integer periodId);

    @Query("SELECT sc FROM ScheduleConflict sc WHERE sc.schedule.id IN :scheduleIds ORDER BY sc.createdAt DESC")
    List<ScheduleConflict> findByScheduleIdsIn(@Param("scheduleIds") List<Integer> scheduleIds);

    @Query("SELECT sc FROM ScheduleConflict sc WHERE sc.schedule.id IN :scheduleIds AND sc.isResolved = false ORDER BY sc.createdAt DESC")
