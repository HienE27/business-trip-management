...
public class AutoSchedulingService {
    ...
    private AutoScheduleResponse runScheduling(AutoScheduleRequestDTO request, boolean save) {
        ...
        // FIX: track which schedules are newly generated by THIS algorithm run.
        // Balance/fairness should reflect only the algorithm's output, not the
        // accumulated existing schedules from prior apply-preview runs.
        // Coverage uses merged (existing + new) so users see total fill rate.
        java.util.Set<Integer> newScheduleIds = new java.util.HashSet<>();
        java.util.Set<String> newScheduleKeys = new java.util.HashSet<>();
        for (Schedule s : createdSchedules) {
            if (s.getId() == null) { // new schedules have null id (not yet persisted)
                newScheduleIds.add(System.identityHashCode(s));
                newScheduleKeys.add(s.getStaff().getId() + "_" + s.getWorkDate() + "_" + s.getShiftType().getId());
            }
        }
        ...
        List<Schedule> fairSchedules = createdSchedules.stream()
                .filter(s -> s.getId() == null
                        || newScheduleKeys.contains(s.getStaff().getId() + "_" + s.getWorkDate() + "_" + s.getShiftType().getId()))
                .toList();
