...
public class SchedulingAlgorithmRunner {
    ...
    public List<Schedule> runFairGreedy(SchedulePeriod period, List<ShiftRequirement> requirements,
                                          List<Staff> activeStaff, boolean save,
                                          AlgorithmConfigService.AlgorithmRuntimeConfig runtimeConfig,
                                          Set<Integer> excludedStaffIds) {
        List<Schedule> createdSchedules = new ArrayList<>();
        ...
        // L04 uses per-specialty pool (spec M05 — chuyen gia phai dung chuyen khoa).
        ...
        final Map<String, Integer> fgFairSharePerType =
                autoSchedulingService.computeFairSharePerTypeWithStaff(requirements, fgStaffPool, activeStaff);
        ...
        while (!currentDate.isAfter(periodEnd)) {
            ...
            for (ShiftRequirement req : todayReqs) {
                ...
                Comparator<Staff> fairnessComparator = Comparator
                        ...
                        .thenComparingInt((Staff s) -> {
                            long typeCount = autoSchedulingService.getStaffCountForKey(s.getId(), fgCapturedKey,
                        ...
                        .thenComparingLong((Staff s) -> {
                            return autoSchedulingService.getStaffCountForKey(s.getId(), fgCapturedKey,
                                    periodData.staffShiftTypeCounts(), capturedFgCounts);
                        })
                        ...
                        .thenComparingLong(s -> autoSchedulingService.getTotalStaffCount(
                                s.getId(), periodData.staffShiftTypeCounts(), capturedFgCounts))
                        // Tier 6: Weekend penalty
                        .thenComparingDouble(s -> {
                            if (!isWeekend) return 0.0;
                            long totalShifts = autoSchedulingService.getTotalStaffCount(
                                    s.getId(), periodData.staffShiftTypeCounts(), capturedFgCounts);
                            return totalShifts * runtimeConfig.getWeekendWeight().doubleValue();
                        });
