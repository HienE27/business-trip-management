...
### 4.1 Trạng thái triển khai hiện tại

- Backend đang dùng `ConflictDetectionService` làm nguồn sự thật cho các rule: nghỉ phép, nghỉ bù, `L01` ↔ `L02`, `L03` ↔ `L04`.
- API lịch trả về `compensationDate`; frontend không nên tự suy luận ngày nghỉ bù nếu đã có dữ liệu từ backend.
- Luồng đổi ca `M02-F04` chỉ hợp lệ khi kỳ lịch đã `PUBLISHED`; gửi yêu cầu và duyệt yêu cầu đều phải bám trạng thái này.
- CRUD trực tiếp trên lịch chỉ nên mở khi `schedule_period.status = DRAFT`; các màn hình `M04` và `M05` hiện đã khóa thao tác thêm/sửa/xóa ở kỳ `PUBLISHED` hoặc `ARCHIVED`.
- Luồng đầy đủ kiểu “Kiểm tra xung đột toàn tháng -> Lưu & Công bố” cho `M04` và `M05` vẫn là phần cần hoàn thiện thêm ở UI/UX, dù backend đã có endpoint kiểm tra xung đột theo kỳ.

### 4.2 Implementation Status theo module

#### M01 — Quản lý nhân sự

- **Đã có**: danh sách nhân sự, active staff, tìm kiếm/lọc, lấy hồ sơ hiện tại, tạo/sửa/xóa mềm, import file nhân sự.
- **Đã có**: seed role hệ thống `ADMIN`, `MANAGER`, `STAFF` và tài khoản mẫu để đăng nhập nhanh.
- **Đã có (M01-F05)**: trang `settings/roles` với ma trận vai trò × quyền, toggle cấp/thu hồi quyền, backend endpoint `GET /api/v1/roles/permissions/matrix` + `POST /api/v1/roles/permissions/toggle`.
- **Cần lưu ý**: tài liệu nghiệp vụ dùng tên vai trò thân thiện, nhưng code và API hiện dùng role kỹ thuật `ADMIN` / `MANAGER` / `STAFF`.
...
- **Đã có**: màn `schedule-summary` với chế độ xem theo tháng/ngày/nhân sự, tải lịch theo `periodId`, xem conflict theo kỳ.
- **Đã có**: export Excel, PDF, và workload (với bộ lọc shift type / nhân sự / khoảng ngày) từ dashboard.
- **Đã có**: dashboard thống kê, thông báo, audit history, workload report và biểu đồ cân bằng tải trực quan.
- **Cần lưu ý**: export thực tế hiện bám theo `periodId`, không phải mọi biến thể lọc báo cáo như mô tả spec đều đã hiện diện đầy đủ.

#### M07 — Tự động sắp xếp lịch
