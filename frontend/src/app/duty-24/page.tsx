...
export default function Duty24Page() {
  ...
  const handleRefresh = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([scheduleData, compData]) => {
        setSchedules((scheduleData ?? []).filter((s) => s.shiftType.id === "L01"));
        setCompensationDays(compData ?? []);
      })
      .catch(() => setMessage("Không thể tải lịch trực 24/24."))
  ...
  // Calendar annotations cho L01: ngày nghỉ bù + ngày có trực
  const calendarAnnotations = useMemo(() => {
    ...
    return Array.from(compByDate.entries()).map(([date, days]) => {
      ...
      return {
        ...
        description: `Ngày nghỉ bù sau ca trực 24/24 — không thể xếp lịch khác`,
      };
    });
  }, [compensationDays]);
  ...
  if (loading && periods.length === 0) {
    return (
      <DashboardShell
        ...
        description="Xếp lịch trực 24/24 theo tháng. Hệ thống tự động tính ngày nghỉ bù sau ca trực."
      >
  ...
  return (
    <DashboardShell
      activeSection="duty-24"
      title="Lịch trực 24/24"
      description="Xếp lịch trực 24/24 theo tháng. Hệ thống tự động tính ngày nghỉ bù theo quy định (T2-T5 nghỉ bù hôm sau, T6/T7 nghỉ bù T3 tuần sau, CN nghỉ bù T2)."
    >
      {message && (
        <div
      ...
      <section className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        {[
          {
            label: "Tổng ca trực 24/24",
            value: schedules.length,
            icon: "emergency",
            accent: "bg-shift-24/30 text-on-shift-24",
          },
          {
            label: "Ngày nghỉ bù",
      ...
      {!selectedPeriod ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-outline-variant bg-surface py-20 gap-4">
          <span aria-hidden="true" className="material-symbols-outlined text-5xl text-outline">emergency</span>
          <p className="text-on-surface-variant">Chọn một kỳ lịch để xem lịch trực 24/24.</p>
        </div>
      ) : loading ? (
        <Skeleton className="h-96 rounded-xl" />
      ) : (
        <ScheduleCalendarSection
          ...
          viewMode={viewMode}
          selectedTab={"L01" satisfies ScheduleTab}
