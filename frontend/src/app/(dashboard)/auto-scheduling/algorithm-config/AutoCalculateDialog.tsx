...
function validateInput(input: AutoCalculateInput): ValidationWarning[] {
  ...
  for (const tid of ["L01", "L02", "L03", "L04"] as ShiftTypeId[]) {
    ...
    // Target per staff check
    if (target < 1) {
      warnings.push({
        type: "error",
        key: `${tid}_target_low`,
        message: `${tid} - Target/người quá thấp (< 1)`,
      });
    }
    if (target > 25) {
      warnings.push({
        ...
        message: `${tid} - Target/người cao (> 25) có thể quá tải`,
      });
    }

    // Eligible staff check
    if (eligible < 2) {
      warnings.push({
        type: "error",
        key: `${tid}_eligible_low`,
        message: `${tid} - Cần tối thiểu 2 người đủ điều kiện`,
      });
    }
    ...
    if (avgPerWeek > 6) {
      warnings.push({
        ...
        message: `${tid} - ${avgPerWeek.toFixed(1)} ca/tuần/người có thể quá tải`,
      });
    }

    // Coverage check
    const minNeeded = Math.ceil((target * eligible) / days);
    if (tid === "L01" && minNeeded < 3) {
      warnings.push({
        type: "info",
        key: `${tid}_coverage_low`,
        message: `${tid} - Coverage thấp, có thể thiếu nhân sự`,
      });
    }
    ...
    if (ratio < 0.5) {
      warnings.push({
        type: "warning",
        key: `${tid}_balance`,
        message: `${tid} - Tỷ lệ eligible/target thấp, cần mở rộng eligibility`,
      });
    }
  }
  ...
  if (input.periodDays < 7) {
    warnings.push({
      ...
      message: "Kỳ lịch quá ngắn (< 7 ngày)",
    });
  }
  ...
  if (input.periodDays > 31) {
    warnings.push({
      type: "warning",
      key: "period_too_long",
      message: "Kỳ lịch dài (> 31 ngày) - cần đánh giá kỹ",
    });
  }

  if (input.periodWeeks < input.periodDays / 7 - 1) {
    warnings.push({
      type: "info",
      key: "week_day_mismatch",
      message: "Số tuần không khớp với số ngày",
    });
  }
          ...
          <section className="bg-surface-container-low rounded-xl p-4 border border-outline-variant">
            ...
            <div className={`grid grid-cols-2 gap-3 ${showAdvanced ? "" : "hidden"}`}>
              <FormInput
                ...
                max={31}
                value={periodDays}
                onChange={(v) => { setPeriodDays(Math.max(1, parseInt(String(v)) || 1)); setActivePreset(null); }}
              ...
              <FormInput
                ...
                max={6}
                value={periodWeeks}
                onChange={(v) => { setPeriodWeeks(Math.max(1, parseInt(String(v)) || 1)); setActivePreset(null); }}
            ...
            {!showAdvanced && (
              <div className="flex items-center gap-4 text-[12px]">
                <span className="flex items-center gap-1.5">
                  <span className="font-mono font-semibold text-primary">{periodDays}</span>
                  <span className="text-on-surface-variant">ngày</span>
                </span>
                <span className="text-outline">·</span>
                <span className="flex items-center gap-1.5">
                  <span className="font-mono font-semibold text-primary">{periodWeeks}</span>
              ...
              <div className="space-y-1">
                ...
                {(errors.length + warnings.length) > 5 && (
                  <p className="text-[11px] text-on-surface-variant italic">
                    +{errors.length + warnings.length - 5} vấn đề khác
                  </p>
                )}
              </div>
            </section>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-surface-container-low border-b border-outline-variant">
                    ...
                    <th className="py-2.5 px-3 font-label-sm text-label-sm text-on-surface-variant uppercase">Đủ điều kiện</th>
                    <th className="py-2.5 px-3 font-label-sm text-label-sm text-on-surface-variant uppercase">Ca/người/kỳ</th>
                ...
                <tbody className="divide-y divide-outline-variant">
                      ...
                      <tr key={tid} className={`hover:bg-surface-container-lowest transition-colors ${hasWarning ? "bg-tertiary-container/10" : ""}`}>
                        ...
                        <td className="py-2 px-3">
                          <input
                            ...
                            className="w-16 h-8 px-2 rounded-lg border border-outline-variant bg-surface-container-lowest text-label-sm text-right font-mono tabular-nums focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                          />
                        </td>
                        <td className="py-2 px-3">
                          <input
                  ...
                  <tr className="bg-primary-fixed/30 font-semibold">
                    ...
                    <td className="py-2 px-3 text-right font-mono tabular-nums">{totalEligible}</td>
