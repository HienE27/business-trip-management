...
export default function ServiceClinicPage() {
  ...
  const handleRefresh = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([scheduleData, compData]) => {
        setSchedules((scheduleData ?? []).filter((s) => s.shiftType.id === "L03"));
        setCompensationDays(compData ?? []);
      })
      .catch(() => setMessage("Không thể tải lịch phòng khám dịch vụ."))
  ...
  const calendarAnnotations = useMemo(() => {
    ...
    return Array.from(compByDate.entries()).map(([date, days]) => {
      ...
      return {
        ...
        description: `Ngày nghỉ bù — không thể xếp lịch phòng khám dịch vụ cho nhân sự này`,
      };
    });
  }, [compensationDays]);
  ...
  if (loading && periods.length === 0) {
    return (
      <DashboardShell
        ...
        description="Xếp lịch phòng khám dịch vụ theo tháng. Nhân sự phụ trách ca khám dịch vụ trong ngày được chọn."
      >
  ...
  return (
    <DashboardShell
      activeSection="service-clinic"
      title="Lịch phòng khám dịch vụ"
      description="Xếp lịch phòng khám dịch vụ theo tháng. Ràng buộc: không trùng lịch phòng khám chuyên gia cùng ngày và không xếp vào ngày nghỉ bù."
    >
      {message && (
        <div
      ...
      <section className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        {[
          {
            label: "Tổng ca PK dịch vụ",
            value: schedules.length,
            icon: "medical_services",
            accent: "bg-shift-service/30 text-on-shift-service",
          },
          {
            label: "Ngày nghỉ bù",
          ...
          },
          {
            ...
            icon: "groups",
            accent: "bg-shift-all-day/20 text-on-shift-all-day",
          },
          {
            label: "Xung đột",
      ...
      {!selectedPeriod ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-outline-variant bg-surface py-20 gap-4">
          <span aria-hidden="true" className="material-symbols-outlined text-5xl text-outline">medical_services</span>
          <p className="text-on-surface-variant">Chọn một kỳ lịch để xem lịch phòng khám dịch vụ.</p>
        </div>
      ) : loading ? (
        <Skeleton className="h-96 rounded-xl" />
      ) : (
        <ScheduleCalendarSection
          ...
          viewMode={viewMode}
          selectedTab={"L03" satisfies ScheduleTab}
