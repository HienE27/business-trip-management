...
export function KPICard({
  ...
  return (
    <div
      className={`flex flex-col justify-between rounded-lg border border-outline-variant bg-surface-container-lowest p-5 shadow-sm hover:bg-surface-container-low transition-colors duration-200 ${className}`}
    >
      <div className="flex items-start justify-between">
        <span className="text-label-sm text-on-surface-variant leading-tight max-w-[120px]">
          {label}
        </span>
        <span
          className={`material-symbols-outlined shrink-0 rounded-md p-1.5 text-[20px] ${config.iconBg} ${config.iconText}`}
      ...
      <div className="mt-2 flex items-baseline gap-2">
        <span className={`font-display-lg text-display-lg ${config.valueColor}`}>
          {value}
        </span>
        {helper && (
