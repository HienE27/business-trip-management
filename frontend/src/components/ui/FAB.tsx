  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-30 flex flex-col items-end gap-2">
        ...
        <div className="flex flex-col gap-2 items-end mb-2 animate-in slide-in-from-bottom-2 duration-200">
          {actions.map((action) => (
            ...
            >
              ...
              <span className="material-symbols-outlined text-[20px] text-blue-800">{action.icon}</span>
            </button>
          ))}
        </div>
      )}
      ...
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="w-14 h-14 rounded-full flex items-center justify-center bg-blue-100 text-blue-800 shadow-lg hover:opacity-90 hover:shadow-xl transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary animate-slide-up"
      >
